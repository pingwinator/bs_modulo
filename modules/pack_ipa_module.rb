...
class PackIpaModule < BaseModule
  ...
  def self.run config
    ...
    output_file_name += "#{app_name}_#{config.runtime.branch}_#{config.runtime.configuration}"
    ...
    if config.profile.identity
      ...
      FileUtils.cd(config.runtime.build_dir) do
        system %Q[xcrun -sdk "#{sdk}" PackageApplication  "#{app_file}" -o "#{ipa_output_path}"  --embed "#{profile_file}"] or fail "xcrun packaging and signing ipa"
      end
    else
      # pack ipa manually
      FileUtils.cd(config.runtime.build_dir) do
